...
import { BEINGS, beingById } from "./catalog";
      ...
      <header className="border-b border-line">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-6 gap-y-3 px-4 py-4 md:px-6">
          <button type="button" onClick={() => go("/")} className="text-left">
            <p className="font-display text-lg tracking-[0.34em] text-fg">SEAL</p>
...
function Command({
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
      <p className="text-xs font-medium uppercase tracking-[0.28em] text-muted">Command</p>
      <h1 className="mt-3 max-w-xl font-serif text-4xl leading-tight tracking-tight md:text-5xl">
        Five beings. Conservative math. One close at a time.
      </h1>
      <p className="mt-4 max-w-2xl text-muted">
        The closing agent for The Christman AI Project. Five beings. Conservative math.
        This desk sits the ask.
      </p>
      <dl className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-4">
        <Stat k="Family TAM" v={usd(total.tam)} />
        <Stat k="Family SAM" v={usd(total.sam)} />
        <Stat k="Family SOM" v={usd(total.som)} />
        <Stat k="Asks on the table" v={usd(total.ask)} />
      </dl>
      <p className="mt-3 text-xs text-faint">
        Haircut {pct(1 - total.haircut)} already taken on every TAM. SAM is {pct(total.samShare)} of
        the cut. SOM is {pct(total.somShare)} of SAM.
      </p>
...
function Ledger({
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
      <p className="text-xs font-medium uppercase tracking-[0.28em] text-muted">Ledger</p>
      <h1 className="mt-3 font-serif text-4xl tracking-tight">The numbers, seated.</h1>
      <p className="mt-3 max-w-2xl text-muted">
        Every dollar on this page has already taken the haircut. We do not present the street TAM
        and then whisper the cut in a footnote.
      </p>
        ...
        <table className="w-full min-w-[720px] text-left text-sm">
                ...
                <tr key={being.id} className="border-t border-line">
                  <td className="px-4 py-3">
                    <button type="button" className="text-left" onClick={() => go(`/beings/${being.id}`)}>
                      ...
                      <span className="block text-xs text-muted">{being.patent}</span>
                    </button>
                  </td>
          ...
          <tfoot>
            <tr className="border-t border-line bg-surface">
              ...
              <td className="px-4 py-3 font-mono text-xs">CUT</td>
      ...
      <p className="mt-4 text-xs text-faint">
        TAM = headline × {pct(total.haircut)}. SAM = TAM × {pct(total.samShare)}. SOM = SAM ×{" "}
        {pct(total.somShare)}. Ask is the round, not a percent of SOM.
      </p>
    </main>
  );
}
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
      <div className="flex flex-col gap-8 md:flex-row md:items-start">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium uppercase tracking-[0.28em] text-muted">{being.tag}</p>
          ...
          <dl className="mt-6 flex flex-wrap gap-x-8 gap-y-3 text-sm">
            <div>
              <dt className="text-xs uppercase tracking-[0.2em] text-faint">Stage</dt>
              <dd className="mt-1 font-mono">{being.stage}</dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-[0.2em] text-faint">Patent</dt>
              <dd className="mt-1 font-mono">{being.patent}</dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-[0.2em] text-faint">Ask</dt>
            ...
            {stamped ? (
              <div>
                <dt className="text-xs uppercase tracking-[0.2em] text-faint">Stamp</dt>
      ...
      <section className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Stat k="TAM" v={usd(v.tam)} hint={`from ${usd(v.tamRaw)} headline`} />
        <Stat k="SAM" v={usd(v.sam)} hint={`${pct(v.samShare)} of TAM`} />
        <Stat k="SOM" v={usd(v.som)} hint={`${pct(v.somShare)} of SAM`} />
      </section>
      <p className="mt-3 text-xs text-faint">Conservative TAM/SAM/SOM. Haircut already taken.</p>

      <div className="mt-10 grid gap-8 md:grid-cols-2">
        <article>
          <h2 className="text-xs font-medium uppercase tracking-[0.24em] text-muted">Thesis</h2>
          <p className="mt-3 max-w-prose">{being.thesis}</p>
        </article>
        <article>
          <h2 className="text-xs font-medium uppercase tracking-[0.24em] text-muted">
            Why this round
          </h2>
      ...
      <section className="mt-12">
        <h2 className="text-xs font-medium uppercase tracking-[0.24em] text-muted">Pipeline</h2>
        <ol className="mt-4 grid gap-3 md:grid-cols-2">
          {STEPS.map((step, i) => (
            <li key={step.id} className="rounded-lg border border-line bg-surface p-4">
              ...
              <p className="mt-2 text-sm">{being.pipeline[step.id]}</p>
            </li>
          ))}
        </ol>
      </section>
...
function Closer({
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
      <p className="text-xs font-medium uppercase tracking-[0.28em] text-muted">Closer</p>
      <h1 className="mt-3 font-serif text-4xl tracking-tight">Walk the close.</h1>
      <p className="mt-3 max-w-2xl text-muted">
        Six steps. Intake, dossier, thesis, math, ask, close. The math does not move because
        someone wants a bigger slide.
      </p>
        ...
        <section className="rounded-xl border border-line bg-surface p-5 md:p-8">
          ...
          {currentStep.id === "math" ? (
            <dl className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <Stat k="TAM" v={usd(v.tam)} hint={`headline ${usd(v.tamRaw)} × ${pct(v.haircut)}`} />
              <Stat k="SAM" v={usd(v.sam)} hint={`${pct(v.samShare)} of TAM`} />
              <Stat k="SOM" v={usd(v.som)} hint={`${pct(v.somShare)} of SAM`} />
            </dl>
          ) : null}
...
function Stat({ k, v, hint }: { k: string; v: string; hint?: string }) {
  return (
    <div className="rounded-lg border border-line bg-surface px-4 py-3">
      <dt className="text-xs uppercase tracking-[0.2em] text-faint">{k}</dt>
